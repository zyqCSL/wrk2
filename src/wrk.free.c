...
static struct {
    ...
    int threads;
    // resolution must be smaller than 1 (1%) to get meaningful 99% tail
    // otherwise equals to taking max of each threads' 99% tail
...
int main(int argc, char **argv) {
    ...
    for (uint64_t i = 0; i < cfg.threads; i++) {
        ...
        printf('before free t\n');
        if(cfg.load_by_script) {
            free(t->throughput_arr);
            free(t->throughput_change_us);
        }
        printf('after free t\n');
    }

    if(cfg.load_by_script) {
        printf('before free load_\n');
        free(load_intervals);
        free(load_rates);
        printf('after free load_\n');
    }
    printf('before free stats\n');
    if(cfg.print_realtime_latency)
        free_stats();
    printf('after free stats\n');
    fclose(pt);
...
void *thread_main(void *arg) {
    ...
    // Yanqi, register load change events
    ...
    // Yanqi, register stats dump event
    aeCreateTimeEvent(loop, thread->stats_report_us/1000, report_stats, thread, NULL);
...
static void init_pstats(int threads) {
    ...
    pstats.percent_resolution = 0.5;  // by default, print data at evey 0.5% granularity
...
static void free_stats() {
    free(pstats.thread_ready);

    for(int i = 0; i < pstats.threads; ++i)
        free(pstats.thread_latency_stats[i]);
    free(pstats.thread_latency_stats);

    for(int i = 0; i < pstats.threads; ++i)
        free(pstats.thread_nginx_stats[i]);
    free(pstats.thread_nginx_stats);
    ...
    for(int i = 0; i < pstats.threads; ++i)
        free(pstats.thread_memc_get_stats[i]);
    free(pstats.thread_memc_get_stats);

    for(int i = 0; i < pstats.threads; ++i)
        free(pstats.thread_memc_find_stats[i]);
    free(pstats.thread_memc_find_stats);

    for(int i = 0; i < pstats.threads; ++i)
        free(pstats.thread_memc_set_stats[i]);
    free(pstats.thread_memc_set_stats);
}
...
static int report_stats(aeEventLoop *loop, long long id, void *data) {
    ...
    if(update) {
        ...
        // printf("tid: %d update pstats done\n", thread->tid);

        pthread_mutex_lock(&pstats.mutex);
        // printf("tid: %d get summary permission\n", thread->tid);
        pstats.ready_ctr += 1;
        pstats.thread_ready[thread->tid] = true;
        // printf("tid: %d pstats ready threads = %d, total threads = %d\n", thread->tid, pstats.ready_ctr, pstats.threads);
        if(pstats.ready_ctr == pstats.threads) {

            // printf("tid: %d pstats ready threads = %d, total threads = %d\n", thread->tid, pstats.ready_ctr, pstats.threads);
            // printf("tid: %d summary pstats\n", thread->tid);
            pstats.ready_ctr = 0;
            for(int i = 0; i < pstats.threads; ++i)
                pstats.thread_ready[i] = false;

            // print stats
            uint64_t latency = get_99_tail(pstats.thread_latency_stats, pstats.threads, pstats.slot, pstats.percent_resolution, start_percent);
            uint64_t nginx = get_99_tail(pstats.thread_nginx_stats, pstats.threads, pstats.slot, pstats.percent_resolution, start_percent);
            uint64_t memc_get = get_99_tail(pstats.thread_memc_get_stats, pstats.threads, pstats.slot, pstats.percent_resolution, start_percent);
            uint64_t memc_find = get_99_tail(pstats.thread_memc_find_stats, pstats.threads, pstats.slot, pstats.percent_resolution, start_percent);
            uint64_t memc_set = get_99_tail(pstats.thread_memc_set_stats, pstats.threads, pstats.slot, pstats.percent_resolution, start_percent);

            // printf("latency = %lu\n", latency);
            // printf("nginx = %lu\n", nginx);
            // printf("memc_get = %lu\n", memc_get);
            // printf("tid: %d summary pstats done\n", thread->tid);
        }

        // uint64_t end = time_us();
        // printf("stats takes time: %lu\n", end - start);
        pthread_mutex_unlock(&pstats.mutex);
    }

    // // debug
    // printf("Throughput changed to %.2f at time %lus\n", thread->throughput, (time_us() - thread->base_start_time)/1000000L );
    // TODO: return time until next stats report in milliseconds
    return (int)(thread->stats_report_us/1000);
}

static uint64_t get_99_tail(uint64_t** stats, int threads, int slot, double resolution, double start_percent) {
    ...
    while(cum_percent < 99.0) {
        ...
        for(int i = 0; i < threads; ++i) {
            if(ptr[i] >= slot)
                continue;
            if(min_thread < 0 || min_val > stats[i][ptr[i]]) {
                min_thread = i;
                min_val = stats[i][ptr[i]];
            }
        }
        // printf("min_thread = %d\n", min_thread);
        // printf("min_val = %lu\n", min_val);
        // for(int i = 0; i < threads; ++i) {
        //     if(ptr[i] < slot)
        //         printf("candidate t%d: %lu\t", i, stats[i][ptr[i]]);
        // }
        // printf("\n");
        // printf("slot = %d\n", slot);
        // for(int i = 0; i < threads; ++i)
        //     printf("t %d : ptr %d\t", i, ptr[i]);
        // printf("\n");
        // if(min_thread < 0 || min_val < cur_val) {
        //     printf("min_val = %lu\n", min_val);
        //     printf("cur_val = %lu\n", cur_val);
        //     for(int i = 0; i < threads; ++i) {
        ...
        assert(min_val >= cur_val);
        cur_val = min_val;
        if(ptr[min_thread] == 0)
            cum_percent += 1.0/threads * start_percent;
        else
